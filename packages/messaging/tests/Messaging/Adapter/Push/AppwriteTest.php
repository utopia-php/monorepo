...
{
    private const string PROJECT = 'project-test';
    private const string CREDENTIAL = 'header.payload.signature';
    ...
    public function testPipelinesPublishesToManyDevices(): void
    {
        ...
        try {
            ...
            $seenTopics = array_map(fn(array $p): string => $p['topic'], $captured['publishes']);
            sort($seenTopics);
            $expectedTopics = array_map(fn(string $t): string => 'appwrite/push/' . $t, $tokens);
            sort($expectedTopics);
