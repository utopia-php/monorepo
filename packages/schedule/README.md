...
- **A successor can tell what its predecessor never saw.** `syncedUntil` records when the source was last read, so coverage and the view that produced it travel together. A schedule created after that read was invisible to the predecessor, so it is covered from when it took effect even though the watermark has moved past it; one that already existed was covered, so it is not replayed. Without that distinction a cold start has to choose between losing the first runs of new schedules and replaying history for every schedule it loads.
