...
- Packages never commit `composer.lock` (the root lock is the QA toolchain and is the only one). Don't add or refresh package locks; `bin/monorepo validate` rejects a tracked one.
